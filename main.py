...
image = (modal.Image.from_registry("nvidia/cuda:12.4.0-devel-ubuntu22.04", add_python="3.12").apt_install(["ffmpeg", "libgl1-mesa-glx", "wget", "libcudnn8", "libcudnn8-dev"]).pip_install_from_requirements("requirements.txt").run_commands(
    ["mkdir -p /usr/share/fonts/truetype/custom", "wget -O /usr/share/fonts/truetype/custom/Anton-Regular.ttf https://github.com/google/fonts/raw/main/ofl/anton/Anton-Regular.ttf", "fc-cache -f -v"]).add_local_dir("asd", "/asd", copy=True))
...
def process_clip(base_dir:str,Original_video_path:str,s3_key:str,start_time:float,end_time:float,clip_index:int,transcript_segments:list):
    ...
    s3_client.upload_file(subtitle_output_path,"ai-podcast-clipper-nandhan",output_s3_key)
...
class AiPodcastClipper:
    ...
    def identify_moments(self,transcript:dict):
        response=self.gemini_client.models.generate_content(model="gemini-2.5-flash-preview-04-17",contents="""This is a podcast video transcript consisting of word, along with each words's start and end time. I am looking to create clips between a minimum of 30 and maximum of 60 seconds long. The clip should never exceed 60 seconds.
    ...
    def process_video(self, request: ProcessVideoRequest, token: HTTPAuthorizationCredentials = Depends(auth_scheme)):
        ...
        try:
            s3_client.head_object(
                Bucket="ai-podcast-clipper-nandhan", Key=s3_key)
            print("✅ Object is accessible")
        except Exception as e:
            import traceback
            traceback.print_exc()

        s3_client.download_file(
            "ai-podcast-clipper-nandhan", s3_key, str(video_path))
        ...
        #3 .Process Clips
        for index,moment in enumerate(clip_moments[:6]):
